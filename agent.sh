...
_write_edit() {
  ...
  local content
  content=$(cat "$path") || {
    echo "Error: cannot read $path. Check the path and read the file first to verify it exists."
    return 1
  }
  ...
  [ "$edit_count" -eq 0 ] && {
    echo "Error: edits array is empty. Provide at least one {oldText, newText} edit."
    return 1
  }
  ...
  for ((i = edit_count - 1; i >= 0; i--)); do
    ...
    [ -z "$oldText" ] && {
      echo "Error: edit $i has empty oldText. oldText must contain the exact text to replace."
      return 1
    }
...
_format_tool_request() {
  ...
  edit | write)
    local path mode
    path=$(echo "$tool_args" | jq -r '.path // "unknown"')
    mode=$(echo "$tool_args" | jq -r '.mode // "edit"')
    echo "📝 File: $path"
    echo "   Mode: $mode"
    if [ "$mode" = "edit" ]; then
      local num_edits
      num_edits=$(echo "$tool_args" | jq '.edits | length // 0')
      echo "   Changes: $num_edits edit(s)"
      # 显示每个 change 的前 3 行 oldText 预览
      local i=0
      while [ "$i" -lt "$num_edits" ]; do
        local old_preview
        old_preview=$(echo "$tool_args" | jq -r ".edits[$i].oldText // \"\" | split(\"\\n\")[:3] | join(\"\\n\")")
        echo "   ---"
        printf '   old:\n'
        echo "$old_preview" | sed 's/^/      /'
        local new_preview
        new_preview=$(echo "$tool_args" | jq -r ".edits[$i].newText // \"\" | split(\"\\n\")[:3] | join(\"\\n\")")
        echo "   new:"
        echo "$new_preview" | sed 's/^/      /'
        i=$((i + 1))
      done
    else
      # patch mode：显示 diff 前 5 行
      local diff_preview
      diff_preview=$(echo "$tool_args" | jq -r '.content // ""' | head -5)
      echo "   Diff preview:"
  ...
  bash)
    local cmd timeout
    cmd=$(echo "$tool_args" | jq -r '.command // "unknown"')
    timeout=$(echo "$tool_args" | jq -r '.timeout // empty')
    echo "💻 Command: $cmd"
    if [ -n "$timeout" ]; then
  ...
  read)
    ...
    file_path=$(echo "$tool_args" | jq -r '.file_path // "unknown"')
    offset=$(echo "$tool_args" | jq -r '.offset // empty')
    limit=$(echo "$tool_args" | jq -r '.limit // empty')
    echo "📄 File: $file_path"
...
_before_tool_call() {
  ...
  write | edit)
    ...
    # 交互确认 — 展示完整请求详情
    {
      echo ""
      echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
      echo "⚠️  Permission request: $tool_name"
      _format_tool_request "$tool_name" "$tool_args"
      echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
      echo ""
      echo "Allow? (y/N): "
    } >&2
...
- edit: Edit or patch a file. Use mode="edit" (default) for precise oldText→newText replacement. Use mode="patch" to apply a unified diff. Each edits[].oldText must match exactly once in the file. For multiple changes in one file, use one edit call with multiple entries in edits[] instead of separate calls.
...
- subagent: Delegate a sub-task to a child agent process. Provides: task (string, required) — the task description to delegate. The child agent runs with the same API configuration and returns the result. Use this for complex multi-step tasks that benefit from parallel or independent processing.
...
- When changing multiple separate locations, use one edit call with multiple entries in edits[] rather than separate calls.
...
'
  ...
  prompt="$prompt"$'\n'"</skills_metadata>"
  local others="Importent informations:Now **date** is $(date),**Current path**(you are work in and project is in) is $(pwd),You must ansower user with **$LANG**"
  prompt="$prompt"$'\n'"$others"
...
execute_tool_call() {
  ...
  if ! _before_tool_call "$tool_name" "$tool_args"; then
    local denied_msg="${_block_reason:-Blocked by permission check.}"
    # stderr 给用户看格式化详情，stdout 给 LLM 看简短说明
    echo "$denied_msg" >&2
    echo "" >&2
    echo "Request was:" >&2
    _format_tool_request "$tool_name" "$tool_args" >&2
    # stdout 返回给 LLM，让它知道发生了什么
    echo "Tool call blocked: $denied_msg"
    return 0
  fi

  # 展示工具调用详情（stderr，不影响 stdout → LLM）
  echo "🔧 $tool_name" >&2
  _format_tool_request "$tool_name" "$tool_args" >&2
  echo "" >&2
  ...
  load_skill)
    ...
    ;;
  subagent)
    local sub_task=$(echo "$tool_args" | jq -r '.task // empty')
    if [ -z "$sub_task" ]; then
      echo "Error: missing required argument 'task'"
      return
    fi
    # 用唯一临时文件名（避免 $$ 在父子进程中不一致的问题）
    local sub_result_file="/tmp/subagent_result_$RANDOM$$$(date +%s%N)"
    # 继承父 agent 的配置，启动子 agent 进程
    # 子 agent 通过 --task 模式运行，结束后将结果写入临时文件
    API_BASE="${API_BASE:?missing API_BASE}" \
    API_KEY="${API_KEY:?missing API_KEY}" \
    MODEL="${MODEL:?missing MODEL}" \
    SUBAGENT_RESULT_FILE="$sub_result_file" \
    bash "$SCRIPT" \
      --subagent \
      --task "$sub_task" \
    ...
    if [ -f "$sub_result_file" ]; then
      sub_output=$(cat "$sub_result_file")
      rm -f "$sub_result_file"
    fi
    result="🤖 Subagent result:
${sub_output:-(subagent task completed)}"
    ;;
  *)
  ...
  while [ $iter -lt $max_iterations ]; do
    ...
    while [ $retry -lt $max_retries ]; do
      ...
      response=$(jq -n \
        ...
        echo "⏭️  Skipping turn, continuing..."
        continue
      }
      rm -f "$_msgfile"
      ...
      # 检查是否是 Engine Busy（可重试错误）
      if echo "$api_error" | grep -qi "busy\|engine.*busy\|service.*busy\|rate.*limit\|throttl"; then
        retry=$((retry + 1))
        if [ $retry -lt $max_retries ]; then
          local wait_sec=$((retry * 2))
          echo "⏳ API busy ($api_error), retrying in ${wait_sec}s ($retry/$max_retries)..."
          sleep "$wait_sec"
          continue
        fi
      fi
    ...
    # API 耗尽所有重试，报最终错误
    if [ -n "$api_error" ]; then
      echo "❌ API error after $max_retries retries: $api_error"
      sleep "$wait_sec"
      echo "⏭️  Skipping turn, continuing..."
      continue
    fi
...
run_agent() {
  ...
  if [ -n "$session_dir" ] && [ -f "$session_dir/messages.jsonl" ]; then
    ...
    # 把新任务追加到 JSONL，让 LLM 能看到
    printf '%s' "$(jq -c -n --arg t "$user_task" '{role:"user",content:$t}')" >>"$session_dir/messages.jsonl"
...
# 子 agent 模式：将最终 assistant 回复写入临时文件供父 agent 读取
if [ "${SUBAGENT_MODE:-}" = "1" ] && [ -n "$SESSION_DIR" ]; then
  last_assistant=$(jq -r 'select(.role=="assistant") | .content' "$SESSION_DIR/messages.jsonl" 2>/dev/null | tail -1)
  if [ -n "$last_assistant" ]; then
    result_file="${SUBAGENT_RESULT_FILE:-/tmp/subagent_result_$$}"
    printf '%s' "$last_assistant" >"$result_file"
  fi
fi
