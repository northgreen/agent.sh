...
TOOLS_JSON='[
  ...
  },
  {
    ...
    "function": {
      "name": "write",
      "description": "Patch a file with **unified diff**. with a unified diff (generated by diff -u) for incremental changes — more token-efficient for large files. Has permission checks: system paths are denied, temp paths are auto-allowed, other paths require user confirmation. Returns operation result.",
      "parameters": {
        "type": "object",
        "properties": {
          "path": {
          ...
          "content": {
            "type": "string",
            "description": "Unified diff to apply (mode=patch, default) "
          },
          "mode": {
            "type": "string",
            "enum": ["patch"],
            "default": "patch",
            "description": "Write mode: 'patch' applies a unified diff incrementally (default)"
          }
        },
...
- write: Write or patch a file (default: patch/unified diff; use mode="write" for full overwrite)
...
## Tool Philosophy
- In write tool , patch is better then write,**use it PLEASE**
...
execute_tool_call() {
    ...
    case "$tool_name" in
        ...
        write)
            ...
            local mode=$(echo "$tool_args" | jq -r '.mode // "patch"')
